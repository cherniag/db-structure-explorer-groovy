...
# Add temporary UITS header to the MP3
	java -jar ${CP}/uits-3.5-SNAPSHOT.jar ${PRIVATE_KEY} "./files/purchased/${ISRC}.mp3" "./${ISRC}.mp3"    || { echo "command failed"; exit 1; } 
...
echo "***** Generating Mobile Audio *****"
	...
	echo "Encoding " ${INPUT} "with ${NERO_DIR}/dmg"

	${NERO_DIR}/dmg --input-file "${INPUT}" --mp4-chunk-span 900 --overwrite --audio-only --repair-all --audio-encoder aac --audio-cbr-rate 48 --aac-mode he-aacv1 --output-file "${ISRC}_48.m4a" --input-speech false --clipmode prolimit|| { echo "command failed"; exit 1; } 
	...
	java -jar ${CP}/uits-3.5-SNAPSHOT.jar ${PRIVATE_KEY} ${ISRC}_48.m4a ${ISRC}_48.aud  ${ISRC}_48.hdr|| { echo "command failed"; exit 1; } 

	
	${NERO_DIR}/dmg --input-file "${INPUT}" --mp4-chunk-span 900 --overwrite --audio-only --repair-all --audio-encoder aac --audio-cbr-rate 96 --aac-mode he-aacv1 --output-file "${ISRC}_96.m4a" --input-speech false --clipmode prolimit|| { echo "command failed"; exit 1; } 
	...
	java -jar ${CP}/uits-3.5-SNAPSHOT.jar ${PRIVATE_KEY} ${ISRC}_96.m4a ${ISRC}_96.aud   ${ISRC}_96.hdr || { echo "command failed"; exit 1; } 
...
echo "***** Generating Mobile Preview Audio *****"
	...
	${NERO_DIR}/dmg --input-file "${INPUT}" --mp4-chunk-span 900 --overwrite --audio-only --repair-all --audio-encoder aac --audio-cbr-rate 96 --aac-mode he-aacv1 --output-file "${ISRC}P.m4a" --input-speech false --clipmode prolimit --trim-end 30000 --audio-fade-interval 1000 || { echo "command failed"; exit 1; }
...
#		${FFMPEG} -i "${ISRC}_96.m4a" -y -t 30 -acodec copy "${ISRC}P.m4a"|| { echo "command failed"; exit 1; } 
	...
	java -jar ${CP}/uits-3.5-SNAPSHOT.jar ${PRIVATE_KEY} "${ISRC}P.m4a" ${ISRC}P.aud  ${ISRC}P.hdr || { echo "command failed"; exit 1; } 
...
# Publish
	...
	do
		curl -X PUT -T files/audio/${i}  -H "X-Auth-Token: ${TOKEN}" -H "X-CDN-Enabled: True" -H "X-TTL: 25000" ${URL}/data/`basename $i`
	done
	
	curl -X PUT -T files/audio/${ISRC}_${BIT_RATE}.aud  -H "X-Auth-Token: ${TOKEN}" -H "X-CDN-Enabled: True" -H "X-TTL: 25000" ${URL}/data/${ISRC}.aud

	for i in `ls files/image`
	do
		curl -X PUT -T files/image/${i}  -H "X-Auth-Token: ${TOKEN}" -H "X-CDN-Enabled: True" -H "X-TTL: 25000" ${URL}/data/`basename $i`
	done
