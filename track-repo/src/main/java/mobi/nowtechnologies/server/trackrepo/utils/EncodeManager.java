    ...
    public void encode(Track track, Boolean isHighRate, Boolean licensed) throws IOException, InterruptedException {
        ...
        try {
            if (licensed) {
                ...
                filesToPrivate.addAll(imageGenerator.generateThumbnails(track.getFileName(AssetFile.FileType.IMAGE), track.getUniqueTrackId(), isVideo));
            }
            else {
                filesToPrivate.addAll(imageGenerator.generateThumbnailsWithWatermark(track.getFileName(AssetFile.FileType.IMAGE), track.getUniqueTrackId(), isVideo));
            }
        }
        catch (Exception e) {
        ...
        if (!isVideo) {
            ...
            for (BitRate currentBitrate : BitRate.values()) {
                ...
                commandAddTag.executeCommand(neroHome.getFile().getAbsolutePath(), bitrateFileName, emptyNull(track.getTitle()), emptyNull(track.getArtist()), emptyNull(track.getAlbum()),
                                             emptyNull(track.getGenre()), emptyNull(track.getYear()), "", emptyNull(track.getCopyright()), emptyNull(track.getUniqueTrackId()), coverImageFileName);
                if (LOGGER.isDebugEnabled()) {
            ...
            commandAddTag.executeCommand(neroHome.getFile().getAbsolutePath(), previewFileName, emptyNull(track.getTitle()), emptyNull(track.getArtist()), emptyNull(track.getAlbum()),
                                         emptyNull(track.getGenre()), emptyNull(track.getYear()), "", emptyNull(track.getCopyright()), emptyNull(track.getUniqueTrackId()), coverImageFileName);
            if (LOGGER.isDebugEnabled()) {
            ...
            if (LOGGER.isDebugEnabled()) {
                LOGGER
                    .debug("Preview file moved to " + getPreviewDir() + track.getUniqueTrackId() + "P.m4a" + " and exists: " + new File(getPreviewDir() + track.getUniqueTrackId() + "P.m4a").exists());
            }
        }
