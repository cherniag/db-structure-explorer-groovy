...
public interface TrackRepository extends JpaRepository<Track, Long>, TrackRepositoryCustom {
    ...
    @Query(
        "SELECT t FROM Track t left join t.territories as ter WHERE t.isrc like :query or t.title like :query or t.artist like :query or t.ingestor like :query or ter.label like :query or ter" +
        ".distributor like :query")
    Page<Track> find(@Param("query") String query, Pageable pageable);

    @Query(
        "SELECT count(t) FROM Track t left join t.territories as ter WHERE t.isrc like :query or t.title like :query or t.artist like :query or t.ingestor like :query or ter.label like :query or " +
        "ter.distributor like :query")
