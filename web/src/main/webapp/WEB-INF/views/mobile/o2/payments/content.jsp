				...
				<div style="padding-top: 16px; padding-left: 10px; padding-bottom: 20px">
					...
					<c:forEach var="paymentPolicy" items="${paymentPolicies}">
						<c:if test="${isIOSDevice && !isO2User}">
							...
							<c:set var="iTunesUrl" value="${pageContext.request.contextPath}/payments/iTunesSubscription.html?paymentPolicyId=${paymentPolicy.id}" />
						</c:if>
					</c:forEach>
			        ...
			        <c:when test="${userIsOptedInToVideo eq false}">
			        	<div class="rel tapArea" style="margin-top: 20px">
			        		<div class="subscription-selector" style="height: 135px;padding: 12px 6px 6px 10px">
                               ...
                               <input class="button-turquoise no-margin pie" title="${pageContext.request.contextPath}/videotrial.html?return_url=payments.html" type="button" onClick="location.href=this.title" value="Yes, I want a free trial" />
				           <!-- </a> -->
				           </div>
			            </div>
			        </c:when>
