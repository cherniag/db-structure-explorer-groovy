...
public class SubscriptionStateFactory {
    ...
    public SubscriptionState getInstance(User user) {
        ...
        boolean paySubscription = (user.isSubscribedStatus() && user.isNextSubPaymentInTheFuture() && (user.getCurrentPaymentDetails() != null));
        ...
        if (paySubscription && !(user.isOnFreeTrial())) {
            ...
            if (!user.isOnFreeTrial()) {
                LOGGER.warn("Expecting user to be on free trial! {} {} {} {} ", user.getUserName(), user.getStatus(), user.getNextSubPayment(), user.getFreeTrialExpiredMillis());
            }
        }
