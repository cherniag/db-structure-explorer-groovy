...
public class PaymentStatusDao {
    ...
    private static void setEntityDao(EntityDao entityDao) {
        List<PaymentStatus> paymentStatusList = entityDao.findAll(PaymentStatus.class);
        ...
        PAYMENT_STATUS_MAP_ID_AS_KEY = Collections.unmodifiableMap(paymentStatusMapIdAsKey);
        ...
        if (AWAITING_PSMS == null) {
            throw new PersistenceException("The parameter AWAITING_PSMS is null");
        }
        if (PSMS_ERROR == null) {
