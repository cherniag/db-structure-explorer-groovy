...
public interface BadgeMappingRepository extends JpaRepository<BadgeMapping, Long> {
    ...
    List<BadgeMapping> findByCommunityResolutionsAndOriginalAlias(@Param("community") Community community, @Param("original") FilenameAlias original,
                                                                  @Param("resolutions") Collection<Resolution> resolutions);
    ...
    @Query(
        value = "select b from BadgeMapping b where b.community=:community and b.originalFilenameAlias.id=:badgeId and (b.resolution=:resolution or b.resolution is null) order by b.resolution desc")
