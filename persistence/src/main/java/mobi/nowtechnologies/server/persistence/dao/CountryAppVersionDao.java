...
public class CountryAppVersionDao extends JpaDaoSupport {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(CountryAppVersionDao.class.getName());
    ...
    public boolean isAppVersionLinkedWithCountry(String appVersion, String countryCode) {
        ...
        if (countryCode == null) {
            throw new PersistenceException("The parameter countryCode is null");
        }
        Long foundedRecordsCount = (Long) getJpaTemplate()
            .find("select count(*) from AppVersion" + " appVersion " + " join appVersion.countries country " + " where country.name=?1 and appVersion.name = ?2", countryCode, appVersion).get(0);
