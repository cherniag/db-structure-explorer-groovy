...
public interface VersionCheckRepository extends JpaRepository<VersionCheck, Long> {
    ...
    @Query(value = "select entity from #{#entityName} entity where " +
                   ...
                   "entity.status in ?8")
    List<VersionCheck> findSuitableVersionsWithQualifier(Community community, DeviceType platform, String applicationName, int majorNumber, int minorNumber, int revisionNumber, String qualifier,
                                                         Collection<VersionCheckStatus> includedStatuses, Pageable pageable);
}
