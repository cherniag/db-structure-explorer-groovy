...
public interface StreamzineUpdateRepository extends JpaRepository<Update, Long> {
    ...
    @Query("select u from Update u where u.date <= ?1 and u.community=?2 order by u.date desc")
    List<Update> findLastSince(Date date, Community community, Pageable pageable);

    @Query("select u from Update u join u.users user where u.date >= ?1 and user = ?2 and u.community=?3 order by u.date asc")
    List<Update> findFirstAfterForUser(Date date, User user, Community community, Pageable pageable);
