...
public class MediaLogDao extends JpaDaoSupport {
    ...
    public void logMediaEvent(int userId, Media media, byte mediaLogType) {
        ...
        if (mediaLogType < 0) {
            throw new IllegalArgumentException("The parameter mediaLogType < 0");
        }
    ...
    public boolean isUserAlreadyDownloadOriginal(int mediaId, int userId) {
        ...
        MediaLogType downloadOriginalMediaLogType = mediaLogTypeDao.getMediaLogTypes().get(MediaLogTypeDao.DOWNLOAD_ORIGINAL);
