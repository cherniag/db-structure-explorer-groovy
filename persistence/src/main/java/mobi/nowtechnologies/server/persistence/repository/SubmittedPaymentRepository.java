...
public interface SubmittedPaymentRepository extends PaymentRepository<SubmittedPayment> {
    ...
    @Query(
        value = "select submittedPayment from SubmittedPayment submittedPayment where submittedPayment.userId in :userIds and submittedPayment.status in :paymentDetailsStatuses order by " +
                "submittedPayment.timestamp desc")
