...
public class PaymentPolicyDao extends JpaDaoSupport {
	...
	public List<PaymentPolicy> getPaymentPoliciesGroupdeByPaymentType(String communityName) {
		...
		List<PaymentPolicy> paymentPolicies = getJpaTemplate().findByNamedQuery("PaymentPolicy.getPaymentPoliciesForCommunity",  communityId);
	...
	public PaymentPolicy getPaymentPolicy(int operatorId, String paymentType, int communityId) {
		...
		if (paymentType.equals(UserRegInfo.PaymentType.PREMIUM_USER)) {
            ...
            paymentPolicies = getJpaTemplate().findByNamedQuery("PaymentPolicy.getPaymentPolicyForNotMigPaymentType",
                    queryArgArray);
        }
	...
	public PaymentPolicy getPaymentPolicy(byte communityId, String paymentType) throws PersistenceException {
		List list = getJpaTemplate().findByNamedQuery("PaymentPolicy.getPaymentPoliciesByCommunityAndPaymentType", paymentType, communityId);
	...
	public List<PaymentPolicy> getPaymentPolicies(String communityURL, boolean availableInStore) {
		...
		List<PaymentPolicy> offerPaymentPolicies = getJpaTemplate().findByNamedQuery(PaymentPolicy.GET_BY_COMMUNITY_AND_AVAILABLE_IN_STORE, community, availableInStore);
