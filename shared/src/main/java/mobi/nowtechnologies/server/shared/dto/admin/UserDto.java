...
public class UserDto {
    ...
    public UserDto(Integer id, String address1, String address2, boolean canContact, String city, String code, int country, String device, String deviceString, byte deviceTypeId, String deviceModel,
                   String deviceType, String displayName, String firstName, byte freeBalance, String ipAddress, Date lastDeviceLogin, String lastName, int lastPaymentTx, Date lastWebLogin,
                   String mobile, Date nextSubPayment, String postcode, String sessionID, byte userStatusId, UserStatus userStatus, int subBalance, String tempToken, String title, String token,
                   byte userGroupId, String userGroup, String userName, UserType userType, int operator, String pin, String paymentType, boolean paymentEnabled, int numPsmsRetries,
                   List<PaymentDetailsDto> paymentDetailsDtos, PaymentDetailsDto currentPaymentDetailsDto, PromotionDto potentialPromotionDto, PromotionDto potentialPromoCodePromotionDto,
                   Integer potentialPromoCodePromotionId, boolean isFreeTrial, String newStoredToken, String conformStoredToken, int paymentStatus, Date lastSuccessfulPaymentTime, String facebookId,
                   String deviceUID, Date firstDeviceLogin, Date firstUserLogin, Long currentPaymentDetailsId, BigDecimal amountOfMoneyToUserNotification, Integer potentialPromotionId,
                   Date lastSuccesfullPaymentSmsSendingTimestamp) {
        super();
        this.id = id;
    ...
    public String toString() {
        return new ToStringBuilder(this).append("id", id).append("address1", address1).append("address2", address2).append("canContact", canContact).append("city", city).append("code", code)
                                        .append("country", country).append("device", device).append("deviceString", deviceString).append("deviceTypeId", deviceTypeId)
                                        .append("deviceModel", deviceModel).append("deviceType", deviceType).append("displayName", displayName).append("firstName", firstName)
                                        .append("freeBalance", freeBalance).append("ipAddress", ipAddress).append("lastDeviceLogin", lastDeviceLogin).append("lastName", lastName)
                                        .append("lastPaymentTx", lastPaymentTx).append("lastWebLogin", lastWebLogin).append("mobile", mobile).append("nextSubPayment", nextSubPayment)
                                        .append("postcode", postcode).append("sessionID", sessionID).append("userStatusId", userStatusId).append("userStatus", userStatus)
                                        .append("subBalance", subBalance).append("tempToken", tempToken).append("title", title).append("token", token).append("userGroupId", userGroupId)
                                        .append("userGroup", userGroup).append("userName", userName).append("userType", userType).append("operator", operator).append("pin", pin)
                                        .append("paymentType", paymentType).append("paymentEnabled", paymentEnabled).append("numPsmsRetries", numPsmsRetries)
                                        .append("paymentDetailsDtos", paymentDetailsDtos).append("currentPaymentDetailsDto", currentPaymentDetailsDto)
                                        .append("potentialPromotionDto", potentialPromotionDto).append("potentialPromoCodePromotionDto", potentialPromoCodePromotionDto)
                                        .append("potentialPromoCodePromotionId", potentialPromoCodePromotionId).append("isFreeTrial", isFreeTrial).append("newStoredToken", newStoredToken)
                                        .append("conformStoredToken", conformStoredToken).append("paymentStatus", paymentStatus).append("lastSuccessfulPaymentTime", lastSuccessfulPaymentTime)
                                        .append("facebookId", facebookId).append("deviceUID", deviceUID).append("firstDeviceLogin", firstDeviceLogin).append("firstUserLogin", firstUserLogin)
                                        .append("currentPaymentDetailsId", currentPaymentDetailsId).append("amountOfMoneyToUserNotification", amountOfMoneyToUserNotification)
                                        .append("potentialPromotionId", potentialPromotionId).append("lastSuccesfullPaymentSmsSendingTimestamp", lastSuccesfullPaymentSmsSendingTimestamp)
                                        .append("freeTrialExpiredMillis", freeTrialExpiredMillis).toString();
    }
}
