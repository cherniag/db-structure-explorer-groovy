...
public class MP4Manager implements MP4ManagerIntf {
    ...
    public int process(String inputFile, String audioFile, String headerFile, String encodedFile, UitsParameters params, String md5, boolean encrypt) throws IOException {
        ...
        FileOutputStream data = new FileOutputStream(audioFile);
        try {
            return process(inputStream, data, new FileOutputStream(headerFile), new FileOutputStream(encodedFile), params, md5, encrypt);
        } finally {
            inputStream.close();
            data.close();
        }
