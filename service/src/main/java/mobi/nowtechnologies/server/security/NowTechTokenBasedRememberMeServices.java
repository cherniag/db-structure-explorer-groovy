...
public class NowTechTokenBasedRememberMeServices extends TokenBasedRememberMeServices {
    ...
    /**
     * Decodes username from cookie tokens to get it back in normal view. Encoding was done in method { makeTokenSignature}
     */
    @Override
    ...
    /**
     * Locates the Spring Security remember me authentication token in the request header and returns its value. The token is searched for by "Remember Me" cookie name. If there was no token found a
    ...
    /**
     * Returns encoded "remember me" authentication token based on username, password and expiration time in milliseconds All parameters should not be null Username will be encoded because it may
     * contain restricted symbols for "remember me" authentication token.
     *
