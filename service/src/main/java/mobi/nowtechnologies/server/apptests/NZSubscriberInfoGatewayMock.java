...
public class NZSubscriberInfoGatewayMock implements NZSubscriberInfoProvider {
    ...
    public static int connectionProblem = 8;
    ...
    public NZSubscriberResult getSubscriberResult(String msisdn) throws MsisdnNotFoundException, ProviderNotAvailableException {
        ...
        if (msisdn.startsWith(notAvailablePrefix)) {
            throw new ProviderNotAvailableException("Connection timeout");
        }

        final String connectionProblemPrefix = vodafoneMsisdnPrefix + NZSubscriberInfoGatewayMock.connectionProblem;
        if (msisdn.startsWith(connectionProblemPrefix)) {
            throw new ProviderConnectionException("Test Reason.");
        }
