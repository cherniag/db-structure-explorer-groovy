...
public class RefundServiceImpl implements RefundService {
    ...
    private Refund logSkippedAudioBoughtPeriodOnTariffMigrationFrom3GTo4GVideoAudio(User userOnOldBoughtPeriod, ActionReason actionReason){
        ...
        if (isOnAudioBoughtPeriod) {
            LOGGER.info("Attempt to log about skipping Audio bought period [{}] on subscription migration to 4G Video Audio. The nextSubPayment was [{}]", isOnAudioBoughtPeriod, userOnOldBoughtPeriod.getNextSubPayment());

            resultRefund = log(userOnOldBoughtPeriod, actionReason);
        } else {
            LOGGER.info("Skip logging about skipped bought period because the following conditions are not met: the user is on Audio bought period [{}], user migrates on 4G Video Audio Subscription", isOnAudioBoughtPeriod);
        }
        return resultRefund;
    }
    ...
    private Refund logSkippedVideoAudioBoughtPeriodOnTariffMigrationFrom4GTo3G(User userWithOldTariffOnOldBoughtPeriod, ActionReason actionReason){
        ...
        if (_4G.equals(olUserTariff) && isOn4gVideoAudioBoughtPeriod) {
            ...
            LOGGER.info("Skip logging about skipped bought period because the following condition are not met: the old tariff [{}] is 4G, new tariff is 3G, user is Audio Video bought period [{}]", olUserTariff, isOn4gVideoAudioBoughtPeriod);
        }
