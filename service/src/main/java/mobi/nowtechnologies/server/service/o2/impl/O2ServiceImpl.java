...
public class O2ServiceImpl implements O2Service {
    ...
    public ProviderUserDetails getProviderUserDetails(String serverO2Url, String token) {
        ...
        return new ProviderUserDetails().withOperator(response.getNode().getFirstChild().getFirstChild().getFirstChild().getNodeValue())
                                        .withContract(response.getNode().getFirstChild().getFirstChild().getNextSibling().getFirstChild().getNodeValue());
    }
    ...
    private O2SubscriberData getSubscriberDataInternal(String originalPhoneNumber) {
        ...
        LOGGER.info("phone:{}, business:{}, contract:{}, provider O2:{}", digitOnlyPhoneNumber, data.isBusinessOrConsumerSegment(), data.isContractPostPay(), data.isProviderO2());
